...
export default {
  ...
  checkCustomPerm(msg) {
    return (
      msg.member.voice?.channel?.permissionOverwrites?.some((perm) => perm.type === 'member' && perm.id === msg.member.id) &&
    ...
    ); // text channel should be in same category when created channel exists
  },
  async run(msg) {
    ...
    if (
      mentioned.permissions.has('MANAGE_GUILD') ||
      mentioned.permissions.has('ADMINISTRATOR') ||
      mentioned.permissions.has('MANAGE_ROLES')
    ) {
    ...
    if (mentioned.voice.channel.id !== msg.member.voice.channel.id)
      return msg.reply(msg.guild.lang.get('commands.kick.notInYourChannel', mentioned));
