...
export default async ({ type, member, newChannel, oldChannel }: Options) => {
  if ((type === 'join' || type === 'switch') && client.myCustomChannels.forJoin.includes(newChannel?.id)) {
    await member.fetch();
    const created = await newChannel.guild.channels.create(newChannel.guild.lang.get('channel.name', member.displayName), {
  ...
  if (
    (type === 'leave' || type === 'switch') &&
    client.myCustomChannels.created.includes(oldChannel?.id) &&
